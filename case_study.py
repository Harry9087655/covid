...
if selected=="Data Cleaning":
    ...
    st.markdown("Here comes a problem, the main dataset has a column named 'Province/State' that is majorly comprised of null values. This means we are unable to concentrate the research on the local scale, therefore instead we just need information about the countries. This results in another issue if we use scatter graphs to plot time against cases in a country, which will come up with a very messy graph due each province/state contains differnent number of cases in one day, resulting in lines overlaying. So we need to work out the total number of cases of a particular country in every day. Given that, we group the dataframe by Country/Region, Date and WHO Region, which are the variables we need later in the analysis. The groupby alse drops unwanted columns 'Province/State'.")
